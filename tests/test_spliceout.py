...
class TestSplitout(unittest.TestCase):

    def test_spliceout(self):

        audio_samples = torch.rand(size=(8, 1, 32000), dtype=torch.float32)
        augment = Compose([
                    SpliceOut(num_time_intervals=10,max_width=100,output_type='dict'),
                ],output_type='dict')
        spliceout_samples = augment(samples=audio_samples, sample_rate=16000).samples.numpy()
    ...
    def test_spliceout_perbatch(self):
        ...
        audio_samples = torch.rand(size=(8, 1, 32000), dtype=torch.float32)
        augment = Compose([
                    SpliceOut(num_time_intervals=10,max_width=100, mode = "per_batch", p=1.0, output_type='dict'),
                ],output_type='dict')
        spliceout_samples = augment(samples=audio_samples, sample_rate=16000).samples.numpy()
    ...
    def test_spliceout_cuda(self):
        ...
        audio_samples = torch.rand(size=(8, 1, 32000), dtype=torch.float32, device= torch.device('cuda')) - 0.5
        augment = Compose([
                    SpliceOut(num_time_intervals=10,max_width=100,output_type='dict'),
                ],output_type='dict')
        spliceout_samples = augment(samples=audio_samples, sample_rate=16000).samples.cpu().numpy()
