...
class HighPassFilter(BaseWaveformTransform):
    ...
    def randomize_parameters(
        ...
        dist = torch.distributions.Uniform(
            low=torch.tensor(
                self.min_cutoff_freq, dtype=torch.float32, device=selected_samples.device
            ),
            high=torch.tensor(
                self.max_cutoff_freq, dtype=torch.float32, device=selected_samples.device
            ),
            validate_args=True,
        )
        self.transform_parameters["cutoff_freq"] = dist.sample(sample_shape=(batch_size,))
    ...
    def apply_transform(self, selected_samples: torch.Tensor, sample_rate: int = None):
        ...
        print(self.transform_parameters["cutoff_freq"])

        for i in range(batch_size):
            selected_samples[i] = highpass_biquad(
                selected_samples[i],
                sample_rate=sample_rate,
                cutoff_freq=self.transform_parameters["cutoff_freq"][i],
            )
